...
public class SimpleVolatileQueue {
	...
	public static void main(String[] args) {
		...
		SimpleVolatileQueue sv = new SimpleVolatileQueue(10);
		sv.run();
		//sv.playwithPow();
	...
	public SimpleVolatileQueue(int size) {
		initBuf(size);
	}
	...
	public void run() {
		...
		List<Result> queuePaddedResults = new ArrayList<>();
		...
		for (int i = 0; i < 5; i++) {
			...
			initBuf(10);
			queueResults.add(
					goGeneric(new ProducerQueueVolatile(reps), new ConsumerQueueVolatile(reps), mushResults));

			initBuf(10);
			linkedBlockingResults.add(
					goGeneric(new ProducerQueueLinkedBlocking(reps), new ConsumerQueueLinkedBlocking(reps), mushResults));

			initBuf(10);
			concurrentLinkedResults.add(
					goGeneric(new ProducerQueueConcurrentLinked(reps), new ConsumerQueueConcurrentLinked(reps), mushResults));

			initBuf(10);
			atomicResults
					.add(goGeneric(new ProducerQueueAtomic(reps), new ConsumerQueueAtomic(reps), mushResults));
			initBuf(10);
			atomicLazyResults.add(
					goGeneric(new ProducerQueueAtomicLazy(reps), new ConsumerQueueAtomicLazy(reps), mushResults));
			initBuf(10);
			atomicLazyMaskYieldResults.add(goGeneric(new ProducerQueueAtomicLazyMaskYield(reps),
					new ConsumerQueueAtomicLazyMaskYield(reps), mushResults));
			initBuf(10);
			atomicLazyMaskResults.add(goGeneric(new ProducerQueueAtomicLazyMask(reps),
		...
		//Stats queuePaddedC = queuePaddedResults.stream().collect(collector);
		...
		/*
		System.out.println("QueueWhilePadded: " + queuePaddedC);
		System.out.printf("VolatilePadded: head:%,d, tail:%,d, finalResult:%,d%n", queuePaddedC.head, queuePaddedC.tail, queuePaddedC.finalResult);
		System.out.println();
		*/
	...
	public Result goGeneric(Callable<Result> producer, Callable<Result> consumer,
	...
	public class ProducerQueueVolatilePadded implements Callable<Result> {
		private int reps;
		public long time;

		public ProducerQueueVolatilePadded(int reps) {
			this.reps = reps;
		}
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				counter = 0;
				int currTail = pad.tail;
				int startIndex = currTail - bufSize;
				while (pad.head <= startIndex) {
					Thread.yield();
				}
				buffer[Math.floorMod(currTail, bufSize)] = i;
				pad.tail = currTail + 1;
	...
	public class ConsumerQueueVolatilePadded implements Callable<Result> {
		private int reps;
		public long time;
		public long numTrue;

		public ConsumerQueueVolatilePadded(int reps) {
			this.reps = reps;
		}
		...
		public Result call() {
			...
			IntStream.range(0, reps).forEach(i -> {
				counter = 0;
				int currHead = pad.head;
				while (currHead >= pad.tail) {
					Thread.yield();
					/*
					 * counter++; if(counter > 1000) { Thread.yield(); counter = 0; }
					 */
